...
<script lang="ts">
    ...
    let entries: { src: string, dst: string | (() => void), active: boolean, id?: number }[] = $state([]);
    let type = $state("string");
    let dst: string | (() => void) = $state('');
    let requests = $state([]);
    let fileInput: HTMLInputElement;
    let clipboardWrapper: HTMLTextAreaElement;

    chrome?.devtools?.network?.onRequestFinished?.addListener((request: HAREntry) => {
        const {url, method, postData, _resourceType: type, status} = (request as Entry).request;
        const newList = [{
            url,
            method,
            type,
...
<div style="display: flex">
    ...
    <div style="flex: 1; border-right: 1px solid; padding: 0.5rem">
        <button aria-label="export" onclick={exportRules} title="export">
            <svg height="24" viewBox="0 0 24 24" width="24" xmlns="http://www.w3.org/2000/svg">
                <g color="currentColor" fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round"
                   stroke-width="1.5">
        ...
        <button aria-label="clipboard" onclick={copyToClipboard} title="clipboard">
            <svg height="24" viewBox="0 0 24 24" width="24" xmlns="http://www.w3.org/2000/svg">
                <g color="currentColor" fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round"
                   stroke-width="1.5">
                    <path d="M18 15v7m3.5-3.5h-7M7 16h4m-4-5h8M6.5 3.5c-1.556.047-2.483.22-3.125.862c-.879.88-.879 2.295-.879 5.126v6.506c0 2.832 0 4.247.879 5.127C4.253 22 5.668 22 8.496 22H11.5m3.992-18.5c1.556.047 2.484.22 3.125.862c.88.88.88 2.295.88 5.126V12"/>
                    <path d="M6.496 3.75c0-.966.784-1.75 1.75-1.75h5.5a1.75 1.75 0 1 1 0 3.5h-5.5a1.75 1.75 0 0 1-1.75-1.75"/>
                </g>
            </svg>
        </button>
        <button aria-label="import" onclick={()=>fileInput.click()} title="import">
            <svg height="24" viewBox="0 0 24 24" width="24" xmlns="http://www.w3.org/2000/svg">
                <g color="currentColor" fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round"
                   stroke-width="1.5">
                    <path d="M15.5 16c-.506.491-2.5 1.8-2.5 2.5m2.5 2.5c-.506-.491-2.5-1.8-2.5-2.5m0 0h8"/>
                    <path d="M11 22h-.273c-3.26 0-4.892 0-6.024-.798a4.1 4.1 0 0 1-.855-.805C3 19.331 3 17.797 3 14.727v-2.545c0-2.963 0-4.445.469-5.628c.754-1.903 2.348-3.403 4.37-4.113C9.095 2 10.668 2 13.818 2c1.798 0 2.698 0 3.416.252c1.155.406 2.066 1.263 2.497 2.35C20 5.278 20 6.125 20 7.818V13"/>
                    <path d="M3 12a3.333 3.333 0 0 1 3.333-3.333c.666 0 1.451.116 2.098-.057A1.67 1.67 0 0 0 9.61 7.43c.173-.647.057-1.432.057-2.098A3.333 3.333 0 0 1 13 2"/>
                </g>
            </svg>
        </button>
        ...
        >
            ...
            <div style="flex-flow: wrap">
                ...
                <label><input bind:group={methods} name="methods" type="checkbox" value="main_frame"/>
                    main_frame</label>
            ...
            {#if type === 'string'}
                <StringComponent bind:value={dst}/>
            {:else if type === 'script'}
                <ScriptComponent bind:value={dst}/>
            {:else if type === 'file'}
                <FileComponent bind:value={dst}/>
            {/if}
            <div>
                <button aria-label="add" title="add" type="submit">
                    <svg height="24" viewBox="0 0 24 24" width="24" xmlns="http://www.w3.org/2000/svg">
                        <path color="currentColor" d="M12 8v8m4-4H8m-5.5 0c0-4.478 0-6.718 1.391-8.109S7.521 2.5 12 2.5c4.478 0 6.718 0 8.109 1.391S21.5 7.521 21.5 12c0 4.478 0 6.718-1.391 8.109S16.479 21.5 12 21.5c-4.478 0-6.718 0-8.109-1.391S2.5 16.479 2.5 12" fill="none" stroke="currentColor"
        ...
        <table class="entries-table" style="border-collapse: collapse;">
            <thead>
            <tr>
                <th>Ursprung</th>
                <th>Quelle</th>
        ...
        <table style="border-collapse: collapse;">
            ...
            {#each requests as request, i}
                <tr>
                    ...
                    <td><span role="button" title={request.url} onclick={()=>setSrc(request)}
                              style="white-space: nowrap; overflow: hidden; text-overflow: ellipsis; max-width: 100rem; display: block">{request.url}</span>
                    </td>
